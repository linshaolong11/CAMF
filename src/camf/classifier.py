"""CAMFClassifier class.

!!! example
    ```python
    import sklearn.datasets
    from camf import CAMFClassifier

    model = CAMFClassifier()
...
from camf.base import (
    create_inference_engine,
    determine_precision,
    initialize_camf_model,
)
from camf.config import ModelInterfaceConfig
from camf.constants import (
    PROBABILITY_EPSILON_ROUND_ZERO,
    SKLEARN_16_DECIMAL_PRECISION,
    XType,
    YType,
)
from camf.preprocessing import (
    ClassifierEnsembleConfig,
    EnsembleConfig,
    default_classifier_preprocessor_configs,
)
from camf.utils import (
    _fix_dtypes,
...
if TYPE_CHECKING:
    ...
    from camf.inference import (
        InferenceEngine,
    )
    from camf.model.config import InferenceConfig
...
class CAMFClassifier(ClassifierMixin, BaseEstimator):
    """CAMFClassifier class."""
    ...
    def __init__(  # noqa: PLR0913
        ...
        *,
        ...
        """A CAMF interface for classification.

        Args:
            n_estimators:
                The number of estimators in the CAMF ensemble. We aggregate the
                 predictions of `n_estimators`-many forward passes of CAMF. Each
            ...
            average_before_softmax:
                ...
                - If `True`, the predictions are averaged before applying the softmax
                  function. Thus, we average the logits of CAMF and then apply the
                  softmax.
            ...
            model_path:
                The path to the CAMF model file, i.e., the pre-trained weights.

                - If `"auto"`, the model will be downloaded upon first use. This
                  defaults to your system cache directory, but can be overwritten
                  with the use of an environment variable `CAMF_MODEL_CACHE_DIR`.
            ...
            device:
                The device to use for inference with CAMF. If `"auto"`, the device
            ...
            ignore_pretraining_limits:
                Whether to ignore the pre-training limits of the model. The CAMF
            ...
            fit_mode:
                Determine how the CAMF model is "fitted". The mode determines how the
                data is preprocessed and cached for inference. This is unique to an
                in-context learning foundation model like CAMF, as the "fitting" is
                technically the
            ...
            memory_saving_mode:
                ...
                automatically batching certain model computations within CAMF to
            ...
            inference_config:
                ...
                See [camf.constants.ModelInterfaceConfig][] for details and options.
    ...
    def fit(self, X: XType, y: YType) -> Self:
        ...
        self.model_, self.config_, _ = initialize_camf_model(
        ...
        if self.n_classes_ > self.interface_config_.MAX_NUMBER_OF_CLASSES:
            raise ValueError(
                ...
                "classes supported by CAMF. Consider using a strategy to reduce "
                "the number of classes. For code see "
                "https://github.com/PriorLabs/camf-extensions/blob/main/src/"
                "camf_extensions/many_class/many_class_classifier.py",
            )
