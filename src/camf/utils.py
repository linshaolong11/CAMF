"""A collection of random utilities for the CAMF models."""
...
from camf.model.loading import download_model, load_model

if TYPE_CHECKING:
    from sklearn.base import TransformerMixin
    from sklearn.pipeline import Pipeline

    from camf.classifier import CAMFClassifier, XType, YType
    from camf.model.config import InferenceConfig
    from camf.model.transformer import PerFeatureTransformer
...
def _user_cache_dir(platform: str, appname: str = "camf") -> Path:
    use_instead_path = (Path.cwd() / ".camf_models").resolve()
...
def load_model_criterion_config(
    ...
    *,
    ...
    if model_path is None:
        USER_CAMF_CACHE_DIR_LOCATION = os.environ.get("CAMF_MODEL_CACHE_DIR", "")
        if USER_CAMF_CACHE_DIR_LOCATION.strip() != "":
            model_dir = Path(USER_CAMF_CACHE_DIR_LOCATION)
        else:
            model_dir = _user_cache_dir(platform=sys.platform, appname="camf")

        model_name = f"camf-{version}-{which}.ckpt"
...
def validate_Xy_fit(
    X: XType,
    y: YType,
    estimator: CAMFRegressor | CAMFClassifier,
    *,
    ...
    if X.shape[1] > max_num_features:
        if not ignore_pretraining_limits:
            raise ValueError(
                ...
                "supported by the CAMF model. Set `ignore_pretraining_limits=True` "
    ...
    if X.shape[0] > max_num_samples:
        if not ignore_pretraining_limits:
            raise ValueError(
                ...
                f" by CAMF. Set `ignore_pretraining_limits=True` to override this "
                f"error!",
            )
        warnings.warn(
