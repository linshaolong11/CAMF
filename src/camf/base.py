...
def initialize_camf_model(
    ...
    if isinstance(model_path, str) and model_path == "auto":
        if which == "classifier":
            model_path = Path("./weights/camf-v2-classifier.ckpt")  # type: ignore
        else:
            model_path = Path("./weights/camf-v2-regressor.ckpt")  
...
def create_inference_engine(  # noqa: PLR0913
    *,
    ...
    """Creates the appropriate CAMF inference engine based on `fit_mode`.
