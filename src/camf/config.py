...
from camf.preprocessing import PreprocessorConfig
...
class ModelInterfaceConfig:
    ...
    reasons (to avoid loading CAMF multiple times). However, these can also be
    ...
    """The method used to shift features during preprocessing for ensembling to emulate
     the effect of invariance to feature position. Without ensembling, CAMF is not
    ...
    the effect of invariance to class order. Without ensembling, CAMF is not
    ...
    the row, counting up for duplicates. This helps CAMF to distinguish between
    ...
    data before passing the data to CAMF. The polynomial features are generated by
    ...
    """The preprocessing applied to the data before passing it to CAMF. See
    ...
    """The preprocessing applied to the target variable before passing it to CAMF for
    ...
    number of classes is larger than this number, CAMF requires an additional step
