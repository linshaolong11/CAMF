...
from mamrl.preprocessing import PreprocessorConfig
...
class ModelInterfaceConfig:
    ...
    reasons (to avoid loading MAMRL multiple times). However, these can also be
    ...
    """The method used to shift features during preprocessing for ensembling to emulate
     the effect of invariance to feature position. Without ensembling, MAMRL is not
    ...
    the effect of invariance to class order. Without ensembling, MAMRL is not
    ...
    the row, counting up for duplicates. This helps MAMRL to distinguish between
    ...
    data before passing the data to MAMRL. The polynomial features are generated by
    ...
    """The preprocessing applied to the data before passing it to MAMRL. See
    ...
    """The preprocessing applied to the target variable before passing it to MAMRL for
    ...
    number of classes is larger than this number, MAMRL requires an additional step
