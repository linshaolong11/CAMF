"""MAMRLClassifier class.

!!! example
    ```python
    import sklearn.datasets
    from mamrl import MAMRLClassifier

    model = MAMRLClassifier()
...
from mamrl.base import (
    create_inference_engine,
    determine_precision,
    initialize_mamrl_model,
)
from mamrl.config import ModelInterfaceConfig
from mamrl.constants import (
    PROBABILITY_EPSILON_ROUND_ZERO,
    SKLEARN_16_DECIMAL_PRECISION,
    XType,
    YType,
)
from mamrl.preprocessing import (
    ClassifierEnsembleConfig,
    EnsembleConfig,
    default_classifier_preprocessor_configs,
)
from mamrl.utils import (
    _fix_dtypes,
...
if TYPE_CHECKING:
    ...
    from mamrl.inference import (
        InferenceEngine,
    )
    from mamrl.model.config import InferenceConfig
...
class MAMRLClassifier(ClassifierMixin, BaseEstimator):
    """MAMRLClassifier class."""
    ...
    def __init__(  # noqa: PLR0913
        ...
        *,
        ...
        """A MAMRL interface for classification.

        Args:
            n_estimators:
                The number of estimators in the MAMRL ensemble. We aggregate the
                 predictions of `n_estimators`-many forward passes of MAMRL. Each
            ...
            average_before_softmax:
                ...
                - If `True`, the predictions are averaged before applying the softmax
                  function. Thus, we average the logits of MAMRL and then apply the
                  softmax.
            ...
            model_path:
                The path to the MAMRL model file, i.e., the pre-trained weights.

                - If `"auto"`, the model will be downloaded upon first use. This
                  defaults to your system cache directory, but can be overwritten
                  with the use of an environment variable `MAMRL_MODEL_CACHE_DIR`.
            ...
            device:
                The device to use for inference with MAMRL. If `"auto"`, the device
            ...
            ignore_pretraining_limits:
                Whether to ignore the pre-training limits of the model. The MAMRL
            ...
            fit_mode:
                Determine how the MAMRL model is "fitted". The mode determines how the
                data is preprocessed and cached for inference. This is unique to an
                in-context learning foundation model like MAMRL, as the "fitting" is
                technically the
            ...
            memory_saving_mode:
                ...
                automatically batching certain model computations within MAMRL to
            ...
            inference_config:
                ...
                See [mamrl.constants.ModelInterfaceConfig][] for details and options.
    ...
    def fit(self, X: XType, y: YType) -> Self:
        ...
        self.model_, self.config_, _ = initialize_mamrl_model(
        ...
        if self.n_classes_ > self.interface_config_.MAX_NUMBER_OF_CLASSES:
            raise ValueError(
                ...
                "classes supported by MAMRL. Consider using a strategy to reduce "
                "the number of classes. For code see "
                "https://github.com/PriorLabs/mamrl-extensions/blob/main/src/"
                "mamrl_extensions/many_class/many_class_classifier.py",
            )
