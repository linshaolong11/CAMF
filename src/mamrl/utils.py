"""A collection of random utilities for the MAMRL models."""
...
from mamrl.model.loading import download_model, load_model

if TYPE_CHECKING:
    from sklearn.base import TransformerMixin
    from sklearn.pipeline import Pipeline

    from mamrl.classifier import MAMRLClassifier, XType, YType
    from mamrl.model.config import InferenceConfig
    from mamrl.model.transformer import PerFeatureTransformer
...
def _user_cache_dir(platform: str, appname: str = "mamrl") -> Path:
    use_instead_path = (Path.cwd() / ".mamrl_models").resolve()
...
def load_model_criterion_config(
    ...
    *,
    ...
    if model_path is None:
        USER_MAMRL_CACHE_DIR_LOCATION = os.environ.get("MAMRL_MODEL_CACHE_DIR", "")
        if USER_MAMRL_CACHE_DIR_LOCATION.strip() != "":
            model_dir = Path(USER_MAMRL_CACHE_DIR_LOCATION)
        else:
            model_dir = _user_cache_dir(platform=sys.platform, appname="mamrl")

        model_name = f"mamrl-{version}-{which}.ckpt"
...
def validate_Xy_fit(
    X: XType,
    y: YType,
    estimator: MAMRLRegressor | MAMRLClassifier,
    *,
    ...
    if X.shape[1] > max_num_features:
        if not ignore_pretraining_limits:
            raise ValueError(
                ...
                "supported by the MAMRL model. Set `ignore_pretraining_limits=True` "
    ...
    if X.shape[0] > max_num_samples:
        if not ignore_pretraining_limits:
            raise ValueError(
                ...
                f" by MAMRL. Set `ignore_pretraining_limits=True` to override this "
                f"error!",
            )
        warnings.warn(
